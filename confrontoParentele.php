<html>
	...
	<?php
		...
		while($riga=mysql_fetch_row($risultato))
		{
			...
			/*inserire str_replace per rimpiazzare eventuali spazi con underscore, così non perdo niente nel passaggio tramite form*/
		}	
		echo("
			  <table border=0 width='100%'>
				<tr>
					<td width='5%' bgcolor='#98FB98'></td>
					<td width='95%'>In verde sono evidenziati gli eventuali record con corrispondenza esatta tra gli elementi della tabella ministeriale e quella locale</td>
				</tr>
				<tr>
				...
				<tr>
					<td width='5%'></td>
					<td width='95%'>Gli eventuali elementi non evidenziati sono quelli privi di corrispondenza tra la tabella ministeriale e quella locale</td>
				</tr>
			  </table>
			  <br><br>
			 ");
		...
		while($riga=mysql_fetch_row($risultatoMinistero)) // per ogni riga estratta dalla tabella del ministero
		{
			...
			while($parentelaLocale=mysql_fetch_row($risultatoLocale))
			{
				if(($parentelaRiferimentoRight!='') and ($parentelaRiferimentoLeft!=''))
				{
					...
					$risultato=mysql_query($query)
						or die("Impossibile effettuare query in like per determinare la corrispondenza parziale tra locale e ministero :".mysql_error());
				}
				else
				{
					$query="select tipoparentela
							...
							where codiceapr4=$codiceRiferimento";
					$risultato=mysql_query($query)
						or die("Impossibile effettuare query per determinare la corrispondenza tra locale e ministero :".mysql_error());
				}
				$i=0;
				...
				if((strlen($parentelaRiferimentoLeft)>0) and (strlen($parentelaRiferimentoRight)>0))
				{
					...
					elseif(strcmp($parentelaRiferimento,$parentelaLocale[0])==0) // se la descrizione della tabella ministeriale è uguale alla tabella della tabella locale, la riga del risultato sarà verde
					{
						echo("<tr>");
		...
		echo("<div align='center'>");
			echo("<form name='cliente' action='conversioneExcel.php' method='POST'>");
	...
	<!-- <div align="center"><a href="conversioneExcel.php"><h3>Esporta risultato in Excel</h3></a></div> -->
	<div align="center"><a href="index.php"><h3>Torna alla pagina iniziale</h3></a></div>
	</body>
</html>